...
public class CarOpinion
{
	...
	public void CalculateDriverOpinions(Driver inDriver)
	{
		...
		if (!this.mDriver.contract.GetTeam().carManager.GetCarForDriver(this.mDriver).HasAllPartSlotsFitted() || !Game.IsActive())
		{
	...
	private DialogRule GenerateDriverComment(Driver inDriver, CarOpinion.Happiness inHappiness, CarOpinion.CommentType inSource)
	{
		...
		int num3 = CarManager.GetOverralBestCarsOfChampionship(team.championship).IndexOf(carForDriver);
		...
		float num5 = Mathf.Abs(num6 - inDriver.GetDriverStats().balance);
		float num7 = Mathf.Abs(num6 - driverForCar.GetDriverStats().balance);
	...
	{
		float num3 = (float)this.mDriverOverallHappiness;
		int num2 = (int)this.mDriverAgainstOtherCarHappiness;
		return (CarOpinion.Happiness)Mathf.RoundToInt((num3 + (float)num2) * 0.5f);
	}
	...
	private CarOpinion.Happiness CalculateDriverOverallHappiness(Driver inDriver)
	{
		Team team = inDriver.contract.GetTeam();
		Car carForDriver = team.carManager.GetCarForDriver(inDriver);
		int num = CarManager.GetOverralBestCarsOfChampionship(team.championship).IndexOf(carForDriver);
		if (num < 4)
		{
	...
	private float GetOtherCarComparisonValue(Driver inDriver)
	{
		...
		{
			global::Debug.LogErrorFormat("GetOtherCarComparisonValue Drivers for Team {0} ID {3}, driver {1} with CarID {2}", new object[]
			{
		...
		Car carForDriver3 = team.carManager.GetCarForDriver(inDriver);
		Car carForDriver2 = team.carManager.GetCarForDriver(driverForCar);
		float inDelta = (float)(carForDriver3.GetStats().statsTotal - carForDriver2.GetStats().statsTotal);
